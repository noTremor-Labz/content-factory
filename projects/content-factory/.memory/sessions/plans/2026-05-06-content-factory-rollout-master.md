...
- **Status:** completed / local FFmpeg binary install still pending for real media runtime
...
- **Changes:** workflow presets, provider adapters, render jobs, worker attempt lifecycle/retries, live job status, package export with normalized `video.mp4`, operator retry/cancel/requeue actions
- **TDD:** ComfyUI executor tests, worker orchestration tests, worker package media normalization tests, API SSE/action tests, UI tests для queue/job detail/export/action flow
